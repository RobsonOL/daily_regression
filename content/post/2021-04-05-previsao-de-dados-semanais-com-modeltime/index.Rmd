...
Abaixo eu analiso uma temos uma série de oferta semanal de gasolina para os estados. Os dados podem ser obtidos [aqui](http://www.eia.gov/dnav/pet/hist/LeafHandler.ashx?n=PET&s=wgfupus2&f=W). 
...
Abaixo vemos as quatro variáveis criadas. O banco de dados pode ser extraído com uso da função `prep`, que prepara a receita na base de treinamento, e da aplicação da função `juice`. 
...
Agora podemos criar um *workflow*, que especifica três objetos: (1) os dados a serem utilizados, (2) o pré-processamento a ser aplicado e (3) o modelo a ser estimado.
...
Uma das funções mais úteis do pacote `Modeltime` é `modeltime_table`, que permite construir listas de modelos que podem ser facilmente manipulados em conjunto. É possível calibrar as estimativas na base de teste, criar medidas de performance, além de produzir previsões para um grande número de modelos de uma única vez.
